...
End-to-end golden-file tests that run the *real* `mockaccino.mockCurrentFile` / `mockaccino.stubCurrentFile` / `mockaccino.mockCurrentFileInline` commands inside the Electron host and compare the generated files to committed references. Data-driven and fixture-discovered, so adding a case needs no test code change.
...
- **Inline mocking** is golden-checked when an `expected/<name>_inline.<inputExt>` file exists (the *whole rewritten source* — input + injected block; see `iface_hpp`/`mixed_hpp`). Because the inline command rewrites the source file itself, the test copies the input into the temp dir and runs against that copy (removed with the dir in teardown), never touching the committed fixture — once via `mockaccino.mockCurrentFileInline` (edits the open buffer, then `doc.save()`) and once via `mockaccino.mockFileInline` (by uri, disk write). The inline block carries no volatile fields, so only CRLF→LF normalization applies. Regenerate these goldens with `node scripts/gen-inline-goldens.js` (after `npm run compile-tests`).
