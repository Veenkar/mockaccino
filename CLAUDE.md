...
A **non-mandatory** suite that proves generated mocks actually *compile and run*. It is not part of `npm run test`/`test:unit` or CI. `node integration/run.js` (= `npm run test:integration`): compiles the extension, runs `Mockaccino` on `integration/src/display.h` + `rng.h` to emit gmock mocks into `integration/generated/`, configures+builds a CMake project with **clang** (GoogleTest via `FetchContent`, so the first run needs network), runs the gmock unit tests via `ctest`, then runs the real `game_of_life` binary. If clang/cmake/ninja/network are absent it prints a skip notice and exits 0; real build/test failures exit non-zero.

The example program is a console Conway's Game of Life: `board`/`engine` are compiled for real (and `board` is unit-tested directly), while `display` and `rng` are the **mocked** dependencies (their real `.c` is not linked into the tests — the generated `*_mock.cc` provides the C-linkage symbols). `display.h`/`rng.h` carry deliberately varied signatures to exercise Mockaccino's parsing paths. `integration/build/` and `integration/generated/` are gitignored. See [integration/README.md](integration/README.md).
