...
This is a **mandatory** suite that proves Mockaccino's output is
...
It is its own script, separate from the regular tests (`npm run test` /
`npm run test:unit`), but it must be run and pass alongside them. It never
skips.
...
the suite fails (exits non-zero) rather than skipping — the toolchain is a
