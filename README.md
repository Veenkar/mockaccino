...
### C++ class mocks (in `_mock.h`)

The same **mock** command also generates gmock **mock classes** for C++ interfaces in the file — there's no separate command and no file-type switch: it mocks free C functions the usual way *and*, when the file declares C++ classes, adds the class mocks to the **same `_mock.h`** (the companion `_mock.cc` is written only when there are C functions). gmock can only override `virtual` methods, so Mockaccino targets interface-like classes. Given:
...
| Setting | Description |
|---|---|
| `cpp.enabled` | Also generate gmock mock classes for C++ interfaces in the file (into the same `_mock.h`, default on). |
...
- Mockaccino mocks **free C functions** (C-linkage symbols) and, in the same run, **C++ interface classes** (gmock mock classes, both written into the single `_mock.h`). The C++ class path covers namespaces, nested classes, and `const`/`noexcept`/`override` methods; **templated classes, operator overloads, and ref-qualified methods are skipped** (MVP).
